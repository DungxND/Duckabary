...
def generate_users(num_users=100):
    ...
    with open('./fakeDataGen/datas/users.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['user_id', 'username', 'firstname', 'lastname', 'email', 'phone', 'address', 'registration_date'])
...
def generate_documents(num_docs=1000):
    ...
    with open('./fakeDataGen/datas/documents.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['document_id', 'title', 'author', 'description', 'publisher', 'publication_date', 'quantity'])
