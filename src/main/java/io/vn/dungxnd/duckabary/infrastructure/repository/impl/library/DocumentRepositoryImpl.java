...
public class DocumentRepositoryImpl implements DocumentRepository {
    ...
    private static final String SELECT_DOCUMENT =
            """
            SELECT d.*,
                   b.isbn, b.publisher, b.language, b.genre,
    ...
    public List<Document> findByAuthor(String author) {
        String sql = SELECT_DOCUMENT + " WHERE d.author LIKE ?";
        List<Document> documents = new ArrayList<>();
        ...
        try (Connection conn = DatabaseManager.getConnection();
            ...
            LoggerUtils.error("Error finding documents by author: " + author, e);
    ...
    private void updateDocument(Connection conn, Document document) throws SQLException {
        String sql =
                """
        UPDATE document
       ...
       """;
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, document.title());
            stmt.setString(2, document.author());
            stmt.setString(3, document.description());
            stmt.setInt(4, document.publishYear());
            stmt.setInt(5, document.quantity());
            stmt.setLong(6, document.id());
            stmt.executeUpdate();
        }
    }
